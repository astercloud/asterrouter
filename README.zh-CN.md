...
每个生产实例在安装时只选择一种角色。角色对应的业务对象、导航和访问边界不同，所以安装后不可切换。
...
访问 `http://localhost:8080/setup`，选择一个部署角色并确认其业务边界后完成初始配置。安装完成后，AsterRouter 会清理可能属于其他部署角色的浏览器旧会话，并要求重新登录后再进入所选后台；相同角色的安装请求可以安全重试，改成其他角色会被拒绝。非交互部署设置 `ASTER_DEPLOYMENT_ROLE=platform`；旧的匹配 `ASTER_PROFILES=platform` 与 `ASTER_DEFAULT_PROFILE=platform` 仍兼容。首次启动会将该选择持久化且不可切换。需要其他角色时部署独立实例。
