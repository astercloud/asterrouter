...
1. `ProviderAccountModel` 保存某个采购账号真实发现或手工配置的上游模型库存。
2. `ProviderAccount.models` 是网关热路径的已启用模型集合。
3. `GatewayModel` 是对客户端稳定、可治理的公共模型标识。
4. `ModelRoute` 显式连接公共模型、路由组、采购账号和上游模型。

支持数据面目录的 compatible 账号通过 `/models` 动态进入库存，默认只预览、不自动启用；管理员确认后一次性同步。AWS、Vertex、Azure 等不能仅靠数据面凭据可靠枚举部署名称的账号使用 `manual/unverified` 库存。自动发现目录中消失的模型标记为 `missing`，保留关联路由证据，不会被静默删除。
...
因此，本方案承诺的是“对每个账号持续维护可追溯的实际库存”，而不是“发布一份看起来很全但无法证明可用的名称表”。支持安全数据面目录时用发现证据更新；否则由管理员维护云部署 ID。
...
- 动态库存：能从账号数据面安全发现时记录发现证据，否则由管理员维护部署 ID；前端不写供应商模型数组。
...
flowchart LR
    ...
    Upstream[Provider data-plane catalog]
...
| 对象 | 单一职责 | 是否进入请求热路径 |
| --- | --- | --- |
| `ProviderConnection` | Provider 类型、Base URL 和连接状态 | 是 |
...
flowchart TD
    ...
    Adapter -- 否，例如 AWS / Vertex / Azure --> Manual[保留手工库存并记录 warning]
...
| `anthropic_compatible` | `GET {base_url}/models`，`x-api-key`，按 `after_id` 分页 | 读取 `data[].id` | 已实现 |
| `gemini_compatible` | `GET {base_url}/models`，`x-goog-api-key`，按 `pageToken` 分页 | `models/{id}` 规范化为 `{id}` | 已实现 |
| `aws_bedrock` | Account 手工维护 model ID / inference profile ID | 不请求管理面目录 | 手工模式 |
| `gcp_vertex` | Account 手工维护 publisher model 与 version | 不请求管理面目录 | 手工模式 |
| `azure_openai` | Account 手工维护 deployment name | 不请求 Azure Management Plane | 手工模式 |

自动发现 adapter 都有 15 秒超时、2 MiB 单页上限和最多 100 页限制。发现请求禁止跟随重定向，避免凭证被转发到其他主机。

## 15. 管理端信息架构

```mermaid
flowchart LR
    Providers[Provider 连接] --> ConnectionCheck[端点配置检查]
...
新建账号允许以空库存保存。Compatible 账号创建成功后弹窗切换为编辑态并立即执行动态发现预览，管理员勾选允许模型后再原子应用；AWS、Vertex、Azure 和私有上游在向导中直接添加手工模型并随 Account 保存，不显示不可用的发现/同步操作。空库存账号没有可建立的 ModelRoute，不会参与请求转发。

Provider 连接页面不提供推荐模型、手工模型、白名单或 Secret 输入；连接健康检查只验证连接配置。Account 健康检查仅在 Provider 支持安全的数据面 discovery adapter 时刷新库存。
...
所有新建公共模型表单只展示 `status=active` 的 `GatewayModel.model_id`。编辑旧 Key、策略、集成、定价或 Route 时，如果原值已不在 active 目录中，界面保留并标记为历史模型，由管理员显式移除；系统不会在一次页面加载中静默改写既有授权。采购价格、第三方账单、缓存探针和能力证据使用所选账号的 `ProviderAccount.models`，而不是公共模型目录或 Provider 连接。

## 16. 兼容与迁移

迁移 `065_provider_account_model_inventory.sql` 与 `068_multi_cloud_protocol_routing.sql`：
...
- 网关热路径和 Provider Account API 的 `models` 启用快照保持兼容。
- 连接级模型和 Secret 字段在 068 中迁入 Account 后删除，不保留运行时双轨。
...
| 数据库迁移 | `backend/migrations/065_provider_account_model_inventory.sql`、`backend/migrations/068_multi_cloud_protocol_routing.sql` |
...
| Provider CRUD 重新引入连接级模型或 Secret | 严格请求解码与 Provider 页面回归测试 |
...
```text
账号数据面发现或管理员手工库存 > 管理员显式启用 > 公共目录注释
```
