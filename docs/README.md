...
> 更新日期：2026-08-10
> 测试事实源：[测试与交付事实源 v1](./test/v1/README.md) · [E2E 场景注册表](./test/v1/scenario-registry.json)

## 1. 一页结论

AsterRouter 只服务企业 AI 使用场景，定位为可私有部署的企业 AI 网关与策略控制平台。它连接企业应用、员工与已授权的 AI 供应商，统一完成接入、访问控制、智能路由、成本治理、用量分析和审计。

公开官网使用 `/`，只说明已经实现的产品能力、路由模型与企业边界；管理控制台 `/console` 和服务门户 `/portal` 才是受身份与权限约束的业务工作面。官网不形成第三套控制面。
...
`/console/model-services/catalog` 是企业模型广场。它借鉴 AKRouter 模型市场中“可比较、可选择、可追溯”的交互，但不引入商户、充值、余额或转售对象：

- 按模型家族、模型或供应线路查看企业已批准的 Provider、供应账号和上游模型。
- 同时展示当前实际采购价、参考价、报价倍率、最新健康检查、24 小时成功率和故障切换证据；缺失证据必须明确显示，不能推测或补零。
- 支持按模型家族、模态、模型、路由组、供应商、协议、标签和可调度状态筛选；默认展示全部线路，避免把尚未完成健康检查的供应资源误判为不存在。
- 选择一份现有路由策略后，可以将供应账号设为同批次优先资源，或加入明确的有序降级批次。
- 所有修改仍写入唯一的路由策略事实源，遵守路由组边界并递增策略版本；模型广场不保存第二份选择状态。保存失败时控件恢复为服务端已保存值。
- 线路详情提供输入、输出、缓存、请求价、报价/结算倍率、采购价来源、可信度、运行证据和策略作用范围，并链接到供应账号、有效成本与策略试算的主要管理入口。
...
当前路由策略以路由组为生效边界，由八个可验证区块组成：

1. 策略标识：名称、说明、路由组和启用状态。
2. 决策偏好：成本、速度、稳定或综合均衡。
3. 决策链：候选范围、硬约束、偏好评分、有序降级和故障切换。
4. 硬约束：模型白名单/黑名单、协议准入、原生协议和价格上限。
5. 低价候选池：自动保留价格前 70% 且至少 2 条、严格保留价格前 30% 且至少 2 条、自定义价格分位或不按价格筛选。
6. 有序资源批次：批次内按偏好排序，本批次全部失败后才进入下一批次。
7. 弹性规则：首字节前故障切换、粘性路由和策略有效期。
8. 决策证据：策略 ID、版本、偏好和资源批次写入路由选择说明。

停用、过期、冷却、熔断、账单硬阻断或不可调度的资源始终不参与候选，这是系统安全基线，不作为可关闭的策略开关。硬约束先过滤候选，决策偏好只对合格候选排序。

内置调度偏好使用中国用户易理解的名称：

| 中文名称 | 英文名称 | 行为 |
| --- | --- | --- |
| 综合均衡 | Balanced | 综合路由优先级、账号优先级、容量余量和配置权重排序 |
| 成本优先 | Cost First | 在质量与稳定性底线内选择预计成本最低线路 |
| 速度优先 | Speed First | 优先容量余量高、当前负载低的资源，适合交互式应用 |
| 稳定优先 | Stability First | 优先非探测状态和管理员明确的稳定线路 |

“综合均衡”不是不透明分数。当前决策说明会记录路由优先级、账号优先级、容量余量、负载比例、熔断状态、策略版本和资源批次，便于管理员核对最终排序。

固定顺序不是第五种评分偏好。管理员通过“有序资源批次”表达强控制顺序：同一批次内按所选偏好评分，只有当前批次全部失败后才进入下一批次。启用动态候选优化时，已通过质量门槛的有效成本决策和资源权重只能在当前批次内调整顺序；关闭后使用稳定排序，任何模式都禁止越过批次边界。

### 7.4 策略绑定与优先级

- 同一路由组可以同时启用多份策略，但只能有一份默认策略；未显式绑定 Workspace Key 的请求只使用该默认策略。
- 网关模型通过 `route_group` 选择策略；模型、协议和资源批次规则在选中策略内继续收紧候选。
- 未配置默认策略时不套用其他非默认策略，沿用模型路由原有的健康、容量、优先级和权重排序，不隐式创建规则。
- 资源分组只负责候选资源管理，不承载或嵌套路由策略。

### 7.5 策略生命周期

```text
创建 v1 -> 启用 / 停用 -> 保存变更并递增版本
```

每次保存都会递增策略版本，并将策略 ID 与版本写入后续请求的路由选择说明。当前版本不提供草稿、历史版本回滚或并发竞速降级，界面和 API 也不暴露这些未实现能力。
