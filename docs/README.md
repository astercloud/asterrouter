...
5. 低价候选池：自动价格分位、仅最低价、自定义价格分位或不按价格筛选。
...
固定顺序不是第五种评分偏好。管理员通过“有序资源批次”表达强控制顺序：同一批次内按所选偏好评分，只有当前批次全部失败后才进入下一批次。粘性路由和动态价格优化只能在当前批次内调整顺序，禁止越过批次边界。

### 7.4 策略绑定与优先级

- 每个路由组同一时间最多存在一份启用策略，服务层校验与数据库唯一索引共同保证该约束。
- 网关模型通过 `route_group` 选择策略；模型、协议和资源批次规则在选中策略内继续收紧候选。
- 未配置启用策略时沿用模型路由原有的健康、容量、优先级和权重排序，不隐式创建规则。
