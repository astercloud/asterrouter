...
func TestEmailTemplateRoutesExposeLowercaseCatalogAndIndependentUpdates(t *testing.T) {
	...
	defaults := request(http.MethodGet, "/api/v1/admin/settings/email-templates/defaults", "")
	...
	catalogResponse := request(http.MethodGet, "/api/v1/admin/settings/email-templates", "")
	...
	if len(catalogEnvelope.Data.Events) != 6 || len(catalogEnvelope.Data.Locales) != 2 {
		t.Fatalf("catalog = %+v", catalogEnvelope.Data)
	}

	updated := request(http.MethodPut, "/api/v1/admin/settings/email-templates/balance_low/en-US", `{"subject":"Custom {{.SiteName}}","html":"<p>{{.Amount}}</p>"}`)
	if updated.Code != http.StatusOK || !strings.Contains(updated.Body.String(), `"customized":true`) {
		t.Fatalf("update status=%d body=%s", updated.Code, updated.Body.String())
	}
	loaded := request(http.MethodGet, "/api/v1/admin/settings/email-templates/balance_low/en-US", "")
	...
	restored := request(http.MethodPost, "/api/v1/admin/settings/email-templates/balance_low/en-US/restore", "")
	...
	invalid := request(http.MethodPut, "/api/v1/admin/settings/email-templates/balance_low/en-US", `{"subject":"{{.Unknown}}","html":"<p>body</p>"}`)
...
func TestEmailTemplateRoutesRequireAuthentication(t *testing.T) {
	handler := newTestHandler(t, RuntimeConfig{AdminToken: "secret"})
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/settings/email-templates", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestSMTPConnectionRouteUsesRequestConfiguration(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/settings/smtp/test-connection", body)
	req.Header.Set("Authorization", "Bearer secret")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
