...
func TestEmailTemplatesListUpdateAndRestore(t *testing.T) {
	...
	if len(catalog.Events) != 3 || len(catalog.Locales) != 2 || len(catalog.Templates) != 6 {
...
func TestEmailTemplateUpdateDoesNotOverwriteConcurrentSettings(t *testing.T) {
	...
	_, err := svc.UpdateEmailTemplate(t.Context(), "quota_limit", "en-US", "Custom {{.SiteName}}", "<p>{{.Limit}}</p>")
...
func TestAuthenticationSettingsRequireSecurePublicBaseURL(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "query", baseURL: "https://router.example.test?application=one", wantErr: true},
...
func TestCompleteSetupPersistsOrganization(t *testing.T) {
	repo := NewMemoryRepository()
	svc := NewService(repo, ServiceOptions{Version: "test", StorageMode: "memory"})

	got, err := svc.CompleteSetup(context.Background(), "  Aster Cloud  ")
	if err != nil {
		t.Fatalf("CompleteSetup() error = %v", err)
	}
	if !got.SetupCompleted || got.SiteName != "Aster Cloud" {
		t.Fatalf("setup settings = %+v", got.PublicSettings)
	}
	values, err := repo.GetAll(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if values[KeySiteName] != "Aster Cloud" || values[KeySetupCompleted] != "true" {
		t.Fatalf("persisted setup settings = %#v", values)
	}
}

func TestCompleteSetupRejectsEmptyOrganization(t *testing.T) {
	repo := NewMemoryRepository()
	svc := NewService(repo, ServiceOptions{Version: "test", StorageMode: "memory"})

	if _, err := svc.CompleteSetup(context.Background(), " \t "); err == nil {
		t.Fatal("CompleteSetup() accepted an empty organization name")
	}
	values, err := repo.GetAll(context.Background())
	if err != nil {
		t.Fatal(err)
	...
	if len(values) != 0 {
		t.Fatalf("invalid setup mutated settings: %#v", values)
	}
}

func TestCompleteSetupCannotBeRepeated(t *testing.T) {
	svc := NewService(NewMemoryRepository(), ServiceOptions{Version: "test", StorageMode: "memory"})
	if _, err := svc.CompleteSetup(context.Background(), "First Organization"); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.CompleteSetup(context.Background(), "Second Organization"); !errors.Is(err, ErrSetupCompleted) {
	...
	if got.SiteName != "First Organization" {
		t.Fatalf("organization was overwritten: %q", got.SiteName)
	}
}

func TestCompleteSetupSerializesConcurrentRequests(t *testing.T) {
	repo := NewMemoryRepository()
	services := []*Service{
		NewService(repo, ServiceOptions{Version: "test", StorageMode: "memory"}),
		NewService(repo, ServiceOptions{Version: "test", StorageMode: "memory"}),
	}
	names := []string{"First Organization", "Second Organization"}
	start := make(chan struct{})
	results := make(chan error, len(services))
	for index, service := range services {
		go func(svc *Service, organizationName string) {
			<-start
			_, err := svc.CompleteSetup(context.Background(), organizationName)
			results <- err
		}(service, names[index])
	}
	close(start)
	...
	got, err := services[0].Admin(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if !got.SetupCompleted || (got.SiteName != names[0] && got.SiteName != names[1]) {
		t.Fatalf("persisted setup is inconsistent: %+v", got.PublicSettings)
	}
}

func TestDemoModeIsReadyWithoutPersistingSetup(t *testing.T) {
	repo := NewMemoryRepository()
	svc := NewService(repo, ServiceOptions{Version: "test", StorageMode: "memory", DemoMode: true})
	got, err := svc.Admin(context.Background())
	if err != nil {
		t.Fatalf("Admin() error = %v", err)
	}
	if !got.SetupCompleted || !got.DemoMode {
	...
	if len(values) != 0 {
		t.Fatalf("demo readiness persisted settings: %#v", values)
	}
}
