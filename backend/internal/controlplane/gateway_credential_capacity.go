...
type CredentialCapacityRequest struct {
	...
	ApplicationConcurrencyLimit int
	EstimatedTokens             int
	Now                         time.Time
...
func (s *Service) TryAcquireGatewayCredentialPermit(ctx context.Context, auth gatewaycore.CanonicalAuthContext, estimatedTokens int) (*GatewayCredentialPermit, string, bool, error) {
	...
	if auth.Limits.QPSLimit <= 0 && auth.Limits.RPMLimit <= 0 && auth.Limits.TPMLimit <= 0 && auth.Limits.ConcurrencyLimit <= 0 && auth.Limits.ApplicationConcurrencyLimit <= 0 {
	...
	limits := gatewayCredentialCapacityLimits{
		...
		concurrency: auth.Limits.ConcurrencyLimit, applicationConcurrency: auth.Limits.ApplicationConcurrencyLimit,
	}
	...
	request := CredentialCapacityRequest{
		LeaseID: "credential_lease_" + randomID(12), ApplicationID: auth.ApplicationID,
		CredentialID: auth.CredentialID, QPSLimit: auth.Limits.QPSLimit, RPMLimit: auth.Limits.RPMLimit, TPMLimit: auth.Limits.TPMLimit,
		ConcurrencyLimit: auth.Limits.ConcurrencyLimit, ApplicationConcurrencyLimit: auth.Limits.ApplicationConcurrencyLimit,
...
func credentialCapacityKey(applicationID, credentialID string) string {
	applicationID = strings.TrimSpace(applicationID)
	credentialID = strings.TrimSpace(credentialID)
	return strconv.Itoa(len(applicationID)) + ":" + applicationID + strconv.Itoa(len(credentialID)) + ":" + credentialID
}

func credentialApplicationCapacityKey(applicationID string) string {
	applicationID = strings.TrimSpace(applicationID)
	return strconv.Itoa(len(applicationID)) + ":" + applicationID
}
...
func validateCredentialCapacityRequest(request CredentialCapacityRequest) error {
	...
	if request.QPSLimit < 0 || request.RPMLimit < 0 || request.TPMLimit < 0 || request.ConcurrencyLimit < 0 || request.ApplicationConcurrencyLimit < 0 || request.EstimatedTokens < 0 {
		return errors.New("credential capacity limits must be non-negative")
	}
	if request.ApplicationConcurrencyLimit > 0 && strings.TrimSpace(request.ApplicationID) == "" {
		return errors.New("application_id is required for application concurrency limits")
	}
	return nil
}
...
func (r *MemoryRepository) AcquireCredentialCapacity(_ context.Context, request CredentialCapacityRequest) (CredentialCapacityLease, string, bool, error) {
	...
	key := credentialCapacityKey(request.ApplicationID, request.CredentialID)
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	concurrency := 0
	applicationConcurrency := 0
	applicationKey := credentialApplicationCapacityKey(request.ApplicationID)
	for id, lease := range r.credentialCapacityLeases {
		if !lease.ExpiresAt.After(request.Now) {
			delete(r.credentialCapacityLeases, id)
			continue
		}
		if credentialApplicationCapacityKey(lease.ApplicationID) == applicationKey {
			applicationConcurrency++
		}
		if credentialCapacityKey(lease.ApplicationID, lease.CredentialID) == key {
			concurrency++
		}
	}
	...
	if request.ApplicationConcurrencyLimit > 0 && applicationConcurrency >= request.ApplicationConcurrencyLimit {
		return CredentialCapacityLease{}, "application_concurrency_exhausted", false, nil
	}
	...
	lease := CredentialCapacityLease{ID: request.LeaseID, ApplicationID: request.ApplicationID, CredentialID: request.CredentialID, ExpiresAt: request.LeaseUntil}
...
func (r *MemoryRepository) ExtendCredentialCapacity(_ context.Context, lease CredentialCapacityLease, now, leaseUntil time.Time) (CredentialCapacityLease, bool, error) {
	...
	if !found || !current.ExpiresAt.After(now) || credentialCapacityKey(current.ApplicationID, current.CredentialID) != credentialCapacityKey(lease.ApplicationID, lease.CredentialID) {
...
func (r *PostgresRepository) AcquireCredentialCapacity(ctx context.Context, request CredentialCapacityRequest) (CredentialCapacityLease, string, bool, error) {
	...
	key := credentialCapacityKey(request.ApplicationID, request.CredentialID)
	if request.ApplicationConcurrencyLimit > 0 {
		applicationKey := credentialApplicationCapacityKey(request.ApplicationID)
		if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtextextended($1, 0))`, applicationKey); err != nil {
			return CredentialCapacityLease{}, "", false, err
		}
	}
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM gateway_credential_capacity_leases WHERE application_id=$1 AND credential_id=$2 AND expires_at<=$3`, request.ApplicationID, request.CredentialID, request.Now); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM gateway_credential_rate_samples WHERE application_id=$1 AND credential_id=$2 AND occurred_at<=$3`, request.ApplicationID, request.CredentialID, request.Now.Add(-gatewayCredentialRateWindow)); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	var concurrency, applicationConcurrency, qps, rpm, tokens int
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM gateway_credential_capacity_leases WHERE application_id=$1 AND credential_id=$2 AND expires_at>$3`, request.ApplicationID, request.CredentialID, request.Now).Scan(&concurrency); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	if request.ApplicationConcurrencyLimit > 0 {
		if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM gateway_credential_capacity_leases WHERE application_id=$1 AND expires_at>$2`, request.ApplicationID, request.Now).Scan(&applicationConcurrency); err != nil {
			return CredentialCapacityLease{}, "", false, err
		}
	}
	if err := tx.QueryRowContext(ctx, `
SELECT COUNT(*) FILTER (WHERE occurred_at>$3), COUNT(*), COALESCE(SUM(estimated_tokens), 0)
FROM gateway_credential_rate_samples
WHERE application_id=$1 AND credential_id=$2 AND occurred_at>$4
`, request.ApplicationID, request.CredentialID, request.Now.Add(-gatewayCredentialQPSWindow), request.Now.Add(-gatewayCredentialRateWindow)).Scan(&qps, &rpm, &tokens); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	reason := ""
	switch {
	...
	case request.ApplicationConcurrencyLimit > 0 && applicationConcurrency >= request.ApplicationConcurrencyLimit:
		reason = "application_concurrency_exhausted"
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO gateway_credential_rate_samples(id, application_id, credential_id, estimated_tokens, occurred_at) VALUES($1,$2,$3,$4,$5)`, "credential_sample_"+randomID(12), request.ApplicationID, request.CredentialID, request.EstimatedTokens, request.Now); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	lease := CredentialCapacityLease{ID: request.LeaseID, ApplicationID: request.ApplicationID, CredentialID: request.CredentialID, ExpiresAt: request.LeaseUntil}
	if _, err := tx.ExecContext(ctx, `INSERT INTO gateway_credential_capacity_leases(id, application_id, credential_id, expires_at, created_at) VALUES($1,$2,$3,$4,$5)`, lease.ID, lease.ApplicationID, lease.CredentialID, lease.ExpiresAt, request.Now); err != nil {
		return CredentialCapacityLease{}, "", false, err
	}
	if err := tx.Commit(); err != nil {
		return CredentialCapacityLease{}, "", false, err
...
WHERE id=$1 AND application_id=$2 AND credential_id=$3 AND expires_at>$5
`, lease.ID, lease.ApplicationID, lease.CredentialID, leaseUntil, now)
	if err != nil {
		return CredentialCapacityLease{}, false, err
	}
