...
func (r *PostgresRepository) QueryAlertEvents(ctx context.Context, query AlertQuery) ([]AlertEvent, error) {
	...
	sqlText := `
SELECT id, type, severity, status, title, summary, resource_type, resource_id, application_id, application_name, gateway_principal_id, gateway_principal_name, external_auth_integration_id, external_subject_reference, dedupe_key,
...
INSERT INTO alert_events(
  id, type, severity, status, title, summary, resource_type, resource_id, application_id, application_name, gateway_principal_id, gateway_principal_name, external_auth_integration_id, external_subject_reference, dedupe_key,
  metadata_json, first_seen_at, last_seen_at, acknowledged_at, acknowledged_by, resolved_at, resolved_by
)
...
func appendAlertEventFilters(clauses *[]string, args *[]any, query AlertQuery) {
	...
	appendSearchFilter(clauses, args, query.Search, []string{"type", "severity", "status", "title", "summary", "resource_type", "resource_id", "dedupe_key", "application_id", "application_name", "gateway_principal_id", "gateway_principal_name", "external_auth_integration_id", "external_subject_reference"})
}
...
func memoryAlertEventMatches(event AlertEvent, query AlertQuery) bool {
	...
	values := []string{event.Type, event.Severity, event.Status, event.Title, event.Summary, event.ResourceType, event.ResourceID, event.DedupeKey, event.ApplicationID, event.ApplicationName, event.GatewayPrincipalID, event.GatewayPrincipalName, event.ExternalAuthIntegrationID, event.ExternalSubjectReference}
