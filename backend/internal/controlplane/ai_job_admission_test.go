...
func TestAIJobAdmissionLimitsAreHierarchicalAndAllowIdempotentReplay(t *testing.T) {
	forEachAIJobRepository(t, func(t *testing.T, repo Repository) {
		t.Run("principal", func(t *testing.T) {
			...
			if err := svc.SetAIJobAdmissionLimits(AIJobAdmissionLimits{Organization: 3, Application: 2, Principal: 1}); err != nil {
				t.Fatal(err)
			}
			authA := aiJobTestAuth("application-a", "principal-a")
			...
			authB := aiJobTestAuth("application-a", "principal-b")
			...
			authC := aiJobTestAuth("application-a", "principal-c")
			if _, _, err := svc.BeginDurableAIJob(context.Background(), authC, aiJobTestRequest("admission-application-limit", "fingerprint-application-limit")); !errors.Is(err, ErrAIJobQueueCapacityExceeded) || !strings.Contains(err.Error(), "application") {
				t.Fatalf("application limit error=%v", err)
			}
			authD := aiJobTestAuth("application-b", "principal-d")
			if _, created, err := svc.BeginDurableAIJob(context.Background(), authD, aiJobTestRequest("admission-organization-third", "fingerprint-organization-third")); err != nil || !created {
				t.Fatalf("third organization job created=%t err=%v", created, err)
			}
			authE := aiJobTestAuth("application-c", "principal-e")
			if _, _, err := svc.BeginDurableAIJob(context.Background(), authE, aiJobTestRequest("admission-organization-limit", "fingerprint-organization-limit")); !errors.Is(err, ErrAIJobQueueCapacityExceeded) || !strings.Contains(err.Error(), "organization") {
				t.Fatalf("organization limit error=%v", err)
			}
			replayed, created, err = svc.BeginDurableAIJob(context.Background(), authA, request)
			if err != nil || created || replayed.ID != first.ID {
...
func TestAIJobAdmissionLimitIsAtomicAcrossUniqueConcurrentRequests(t *testing.T) {
	forEachAIJobRepository(t, func(t *testing.T, repo Repository) {
		...
		auth := aiJobTestAuth("application-concurrent-limit", "principal-concurrent-limit")
...
func TestAIJobAdmissionCapacityReturnsWhenJobLeavesQueue(t *testing.T) {
	forEachAIJobRepository(t, func(t *testing.T, repo Repository) {
		...
		auth := aiJobTestAuth("application-capacity", "principal-capacity")
