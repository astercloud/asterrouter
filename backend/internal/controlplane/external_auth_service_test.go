...
func TestExternalAuthContextAuthenticatesWithinIntegrationCeilingAndSnapshotsEvidence(t *testing.T) {
	...
	created, err := svc.CreateExternalAuthIntegration(ctx, "operator", ExternalAuthIntegrationRequest{
		ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID,
	...
	if auth.APIKey.ID == "" || auth.APIKey.ID == created.Record.ID || auth.APIKey.QPSLimit != 4 || auth.APIKey.MonthlyTokenLimit != 400 {
	...
	usage, err := svc.UsageReportQuery(ctx, UsageQuery{ExternalAuthIntegrationID: created.Record.ID})
	if err != nil || len(usage.Recent) != 1 || usage.Recent[0].ExternalSubjectReference != claims.SubjectReference {
		t.Fatalf("usage=%+v err=%v", usage, err)
	}
	traces, err := svc.ListGatewayTracesQuery(ctx, GatewayTraceQuery{ExternalAuthIntegrationID: created.Record.ID})
	if err != nil || len(traces) != 1 || traces[0].ExternalSubjectReference != claims.SubjectReference {
		t.Fatalf("traces=%+v err=%v", traces, err)
	}
	audit, err := svc.ListAuditLogsQuery(ctx, AuditLogQuery{ExternalAuthIntegrationID: created.Record.ID})
	...
	alerts, err := svc.ListAlertEventsQuery(ctx, AlertQuery{ExternalAuthIntegrationID: created.Record.ID})
...
func TestExternalAuthContextFailsClosedForInvalidOrExpandedClaims(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wrong application", mutate: func(c *ExternalAuthContextClaims) { c.ApplicationID = "app_other" }},
	...
	if _, err := svc.UpdateGatewayPrincipal(ctx, "operator", identity.principal.ID, GatewayPrincipalRequest{ApplicationID: identity.application.ID, Name: identity.principal.Name, PrincipalType: identity.principal.PrincipalType, Status: GatewayPrincipalStatusDisabled}); err != nil {
		t.Fatal(err)
	}
...
func TestExternalAuthIntegrationLifecyclePreservesSecretAndRejectsInvalidBinding(t *testing.T) {
	...
	if _, err := svc.CreateExternalAuthIntegration(ctx, "operator", ExternalAuthIntegrationRequest{
		ApplicationID: identity.application.ID, GatewayPrincipalID: "missing", Name: "bad", KeyID: "bad", Audience: "aud", ModelAllowlist: []string{"model"}, QPSLimit: 1, MonthlyTokenLimit: 1,
	}); err == nil {
		t.Fatal("accepted missing gateway principal")
	}
	created, err := svc.CreateExternalAuthIntegration(ctx, "operator", ExternalAuthIntegrationRequest{
		ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID, Name: "Product", KeyID: "product-key", Audience: "aud", ModelAllowlist: []string{"model"}, QPSLimit: 1, MonthlyTokenLimit: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	updated, err := svc.UpdateExternalAuthIntegration(ctx, "operator", created.Record.ID, ExternalAuthIntegrationRequest{
		ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID, Name: "Product renamed", KeyID: "product-key", Audience: "aud", ModelAllowlist: []string{"model"}, QPSLimit: 1, MonthlyTokenLimit: 1, Status: ExternalAuthIntegrationStatusDisabled,
	})
	...
	if _, err := svc.UpdateExternalAuthIntegration(ctx, "operator", created.Record.ID, ExternalAuthIntegrationRequest{ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID, Name: "Product", KeyID: "product-key", Audience: "aud", ModelAllowlist: []string{"model"}, QPSLimit: 1, MonthlyTokenLimit: 1, Secret: "should-not-update"}); err == nil {
		t.Fatal("update accepted secret rotation bypass")
	}
	if _, err := svc.UpdateExternalAuthIntegration(ctx, "operator", created.Record.ID, ExternalAuthIntegrationRequest{ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID, Name: "Product", KeyID: "changed-key-id", Audience: "aud", ModelAllowlist: []string{"model"}, QPSLimit: 1, MonthlyTokenLimit: 1}); err == nil {
...
func TestExternalJWTAuthenticatesWithinIntegrationCeilingAndFailsClosed(t *testing.T) {
	...
	if _, err := svc.UpdateExternalAuthIntegration(ctx, "operator", created.Record.ID, ExternalAuthIntegrationRequest{
		ApplicationID: identity.application.ID, GatewayPrincipalID: identity.principal.ID, Name: "JWT product", Protocol: ExternalAuthIntegrationProtocolJWT, KeyID: "jwt-product-v1",
...
func createExternalAuthIdentity(t *testing.T, ctx context.Context, svc *Service) externalAuthTestIdentity {
	...
	application, err := svc.CreateApplication(ctx, "operator", ApplicationRequest{Name: "External Product", Slug: "external-product"})
	if err != nil {
		t.Fatal(err)
	}
	principal, err := svc.CreateGatewayPrincipal(ctx, "operator", GatewayPrincipalRequest{ApplicationID: application.ID, Name: "Product backend", PrincipalType: GatewayPrincipalTypeIntegration})
	if err != nil {
		t.Fatal(err)
	}
	return externalAuthTestIdentity{application: application, principal: principal}
}
...
func validExternalAuthClaims(integration ExternalAuthIntegration, subject string, now time.Time) ExternalAuthContextClaims {
	return ExternalAuthContextClaims{
		...
		ApplicationID: integration.ApplicationID, SubjectReference: subject, Audience: integration.Audience,
