...
func builtinPlugins(now time.Time) []Plugin {
	return []Plugin{
		...
		builtin(OpenAICompatibleProviderPluginID, "OpenAI-compatible Provider", "Forward compatible text traffic and execute image generation; video and audio adapters can be installed as provider plugins.", "provider", "backend", TierFreeCore, StatusEnabled, EntitlementFree, []string{"personal", "relay_operator", "enterprise"}, "/providers", true, now),
		builtin(ArtifactS3SinkPluginID, "S3-compatible Artifact Delivery", "Deliver generated image, video, and audio artifacts to customer-owned S3, Cloudflare R2, or OSS storage.", "artifact_sink", "integration", TierFreeCore, StatusDisabled, EntitlementFree, []string{"personal", "relay_operator", "enterprise", "platform"}, "/plugins", true, now),
