...
func builtinPlugins(now time.Time) []Plugin {
	return []Plugin{
		...
		builtin(OpenAICompatibleProviderPluginID, "OpenAI-compatible Provider", "Forward chat completion traffic and execute durable image generation with compatible providers.", "provider", "backend", TierFreeCore, StatusEnabled, EntitlementFree, []string{"personal", "relay_operator", "enterprise"}, "/providers", true, now),
