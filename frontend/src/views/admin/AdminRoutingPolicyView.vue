...
import { createRoutingPolicy, getProcurementPrices, getProviderAccounts, getRoutingPolicies, updateRoutingPolicy } from '@/api/control'
import type {
  ProviderAccount,
  ProcurementPrice,
  RoutingPolicy,
...
async function load(preferredID = selectedID.value) {
  ...
  error.value = ''
  try {
    const [policyData, accountData, priceData] = await Promise.all([getRoutingPolicies(), getProviderAccounts(), getProcurementPrices()])
...
function removeBatch(index: number) {
  form.strategy.resource_batches.splice(index, 1)
}
...
<style scoped>
.routing-policy-page { display: grid; gap: 20px; padding-bottom: 96px; }
...
.save-bar { position: fixed; z-index: 20; right: 24px; bottom: 18px; left: calc(256px + 24px); display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 12px 16px; border: 1px solid var(--border-strong); background: color-mix(in srgb, var(--surface) 92%, transparent); box-shadow: var(--shadow-md); backdrop-filter: blur(12px); }
...
@media (max-width: 640px) {
  .routing-policy-page { gap: 14px; padding-bottom: 112px; }
  ...
  .save-bar { right: 10px; bottom: 10px; left: 10px; }
