...
<template>
  <main class="content effective-pricing-page">
    ...
    <div v-if="selectedRow" class="drawer-backdrop" @click.self="selectedRow = null"><aside class="evidence-drawer" role="dialog" aria-modal="true"><header><div><h2>{{ selectedRow.provider_name || selectedRow.provider_id }}</h2><p>{{ selectedRow.provider_account_name || selectedRow.provider_account_id }} · {{ selectedRow.upstream_model }}</p></div><button class="icon-button" type="button" :aria-label="t('common.close')" @click="selectedRow = null"><X :size="18" /></button></header><div class="evidence-grid"><div><small>{{ t('effectivePricing.quoted') }}</small><strong>{{ formatMultiplier(selectedRow.quoted_multiplier) }}</strong></div><div><small>{{ t('effectivePricing.effective') }}</small><strong>{{ formatMultiplier(selectedRow.effective_multiplier) }}</strong></div><div><small>{{ t('effectivePricing.effectiveCost') }}</small><strong>{{ formatMoneyMicros(selectedRow.effective_cost_micros_per_1m, selectedRow.currency) }}</strong></div><div><small>{{ t('effectivePricing.confidence') }}</small><strong>{{ selectedRow.cost_confidence }}</strong></div><div><small>{{ t('effectivePricing.cacheHit') }}</small><strong>{{ formatPercent(selectedRow.cache_token_hit_rate) }}</strong></div><div><small>{{ t('effectivePricing.affinity') }}</small><strong>{{ formatPercent(selectedRow.affinity_consistency_rate) }}</strong></div><div><small>{{ t('effectivePricing.errorRate') }}</small><strong>{{ formatPercent(selectedRow.error_rate) }}</strong></div><div><small>{{ t('effectivePricing.p95Latency') }}</small><strong>{{ formatLatency(selectedRow.p95_latency_ms) }}</strong></div></div><div class="evidence-section"><h3>{{ t('effectivePricing.evidence') }}</h3><p>Price: {{ selectedRow.price_id || '-' }}</p><p>Window: {{ formatDate(report?.window_start) }} → {{ formatDate(report?.window_end) }}</p><p>Reasons: {{ selectedRow.reason_codes.join(' · ') || '-' }}</p></div><footer><button class="button secondary" type="button" @click="openBillingDialog(selectedRow); selectedRow = null">{{ t('effectivePricing.importBill') }}</button><button class="button" type="button" @click="openPriceDialog(selectedRow); selectedRow = null">{{ t('effectivePricing.addPrice') }}</button></footer></aside></div>
...
.evidence-drawer { position: absolute; inset: 0 0 0 auto; width: min(500px,100%); overflow-y: auto; padding: 22px; background: var(--surface); box-shadow: var(--shadow-lg); }
...
.evidence-drawer footer { display: flex; justify-content: flex-end; gap: 8px; }
