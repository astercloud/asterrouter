...
const form = reactive<AdminSettings>({
  ...
  login_agreement_enabled: false,
	...
	default_rpm: 0,
	auth_source_defaults: Object.fromEntries(['local','oidc','feishu','dingtalk','github','google'].map(source => [source, { enabled: false, balance_cents: 0, concurrency: 5, rpm: 0 }])),
	smtp_host: '',
	smtp_port: 587,
	smtp_username: '',
	smtp_password: '',
...
<template>
  <main class="content">
    ...
    <section class="grid section-gap settings-content-grid">
      ...
      <div v-if="activeSettingsTab === 'defaults'" class="panel"><div class="panel-header"><ShieldCheck :size="18"/><h2>按认证来源默认值</h2></div><div class="panel-body auth-provider-list"><section v-for="source in ['local','oidc','feishu','dingtalk','github','google']" :key="source" class="auth-provider-card"><div class="auth-provider-header"><div><strong>{{ source }}</strong><p>覆盖该身份源首次创建用户时的全局默认值。</p></div><label class="switch"><input v-model="form.auth_source_defaults[source].enabled" type="checkbox"/><span></span></label></div><div v-if="form.auth_source_defaults[source].enabled" class="auth-provider-config auth-credential-grid"><div class="field"><label>余额（分）</label><input v-model.number="form.auth_source_defaults[source].balance_cents" type="number" min="0"/></div><div class="field"><label>并发</label><input v-model.number="form.auth_source_defaults[source].concurrency" type="number" min="0"/></div><div class="field"><label>RPM</label><input v-model.number="form.auth_source_defaults[source].rpm" type="number" min="0"/></div></div></section></div></div>
