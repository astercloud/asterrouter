...
import { apiKeyLifecycleClass, apiKeyLifecycleLabelKey, canDisableAPIKey, canRotateAPIKey } from '@/utils/apiKeys'
...
<template>
  <main class="content crud-page">
    ...
    <section class="panel table-panel"><div class="panel-body table-scroll"><table class="data-table crud-table"><thead><tr><th>{{ t('apiKeys.name') }}</th><th>{{ t('operatorDomain.customer') }}</th><th>{{ t('apiKeys.models') }}</th><th>{{ t('providers.status') }}</th><th>{{ t('common.actions') }}</th></tr></thead><tbody><tr v-for="item in filtered()" :key="item.id"><td><strong>{{ item.name }}</strong><span>{{ item.fingerprint }}</span></td><td>{{ customerName(item.customer_id) }}</td><td>{{ item.model_allowlist.join(', ') }}</td><td><span class="pill" :class="apiKeyLifecycleClass(item)">{{ t(apiKeyLifecycleLabelKey(item)) }}</span></td><td class="table-actions"><button class="icon-button" :title="t('operatorDomain.rotateKey')" :disabled="!canRotateAPIKey(item)" @click="rotate(item)"><RotateCw :size="16"/></button><button v-if="canDisableAPIKey(item)" class="icon-button" :title="t('operatorDomain.disableKey')" @click="disable(item)"><ShieldOff :size="16"/></button></td></tr><tr v-if="!filtered().length"><td colspan="5" class="empty-cell"></td></tr></tbody></table></div></section>
  </main>
</template>
