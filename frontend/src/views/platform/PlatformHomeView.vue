...
import { Activity, Fingerprint, KeyRound, PlugZap, RefreshCw, Server } from '@lucide/vue'
...
const metrics = [
  { label: 'dashboard.providers', value: () => dashboard.value?.provider_count ?? 0, sub: () => `${dashboard.value?.active_provider_count ?? 0} active`, icon: Server },
  { label: 'dashboard.apiKeys', value: () => dashboard.value?.api_key_count ?? 0, sub: () => `${dashboard.value?.active_api_key_count ?? 0} active`, icon: KeyRound },
  { label: 'dashboard.models', value: () => dashboard.value?.models.length ?? 0, sub: () => t('dashboard.gatewayCatalog'), icon: PlugZap },
  { label: 'dashboard.recentAudit', value: () => dashboard.value?.recent_audit.length ?? 0, sub: () => t('audit.events'), icon: Fingerprint }
]
