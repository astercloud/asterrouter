...
describe('API module contracts', () => {
  ...
  it('uses the authentication endpoint contracts', async () => {
    ...
    expect(client.post).toHaveBeenLastCalledWith('/auth/verify-email', { token: 'verify-token' })
		await auth.resendVerification('user@example.com', 'resend-turnstile')
		expect(client.post).toHaveBeenLastCalledWith('/auth/resend-verification', { email: 'user@example.com', turnstile_token: 'resend-turnstile' })
		await auth.forgotPassword('user@example.com', 'forgot-turnstile')
		expect(client.post).toHaveBeenLastCalledWith('/auth/forgot-password', { email: 'user@example.com', turnstile_token: 'forgot-turnstile' })
  ...
  it('uses account security endpoint contracts', async () => {
    ...
    expect(client.put).toHaveBeenLastCalledWith('/account/password', { current_password: 'current', new_password: 'new-password-123' })
		await account.beginTOTPSetup('current-password')
		expect(client.post).toHaveBeenLastCalledWith('/account/totp/setup', { current_password: 'current-password' })
    ...
    client.post.mockResolvedValueOnce({ data: { access_token: 'replacement', expires_at: '2099-01-01T00:00:00Z', codes: ['code-1'] } })
		expect((await account.generateTOTPRecoveryCodes('123456')).codes).toEqual(['code-1'])
		expect(client.post).toHaveBeenLastCalledWith('/account/totp/recovery-codes', { code: '123456' })
