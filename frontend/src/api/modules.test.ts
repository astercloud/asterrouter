...
describe('API module contracts', () => {
  ...
  it('uses account security endpoint contracts', async () => {
    ...
    client.post.mockResolvedValueOnce({ data: { codes: ['code-1'] } })
    expect(await account.generateTOTPRecoveryCodes()).toEqual(['code-1'])
    ...
    expect(await account.beginAccountIdentityBinding('github / enterprise')).toBe('https://identity.test/authorize')
    expect(client.post).toHaveBeenLastCalledWith('/account/identities/github%20%2F%20enterprise/bind')
  })
