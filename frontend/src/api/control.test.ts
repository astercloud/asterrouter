...
describe('control API contracts', () => {
  ...
  it('uses admin CRUD endpoint contracts', async () => {
    ...
    const cases: Array<{ run: () => Promise<unknown>; method: ClientMethod; args: unknown[] }> = [
      ...
      { run: () => control.simulateGatewayRouting('model-a', 123), method: 'post', args: ['/console/gateway-simulator', { model: 'model-a', estimated_tokens: 123, protocol: 'openai_chat_completions', required_features: [], routing_policy_id: '' }] },
